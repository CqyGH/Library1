...
//                    alert(BookID);
    ...
    if (BookID == "") {
        ...
        var tbodyObj = document.getElementById('BookList');
        $("table :checkbox").each(function (key, value) {
            if ($(value).prop('checked')) {
                logout = tbodyObj.rows[key].cells[7].innerHTML;
            }
        })
//        alert(logout);
        if (logout == "否") {
...
//用户注销图书
function LogOutAction(Logout, BookID) {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '../../Handlers/UpdateBookLogout.ashx',
        data: {
            BookID: BookID,
            Logout: Logout
        },
